...
class Admin_model extends CI_Model {
    ...
    public function get_search_data() {
        ...
        $this->db->select('in.id as inc_id,mo.model_name,mo.id,in.start_date,in.end_date,  (SELECT GROUP_CONCAT(aes_decrypt(display_name,salt) SEPARATOR "/") FROM emts_members m join emts_dealer_incentive i on(m.id=i.dealer_id) where in.id=i.incentive_id) as dealer_names,in.update_Date as modified_date');
...
//added by shiva
    ...
    $this->db->select('count(S.id) as sales_count,M.display_name');
    $this->db->from('sales_report S');
    $this->db->join('products P','S.product_id = P.id');
    $this->db->join('model M','M.id = P.model_number');
    // if($dealer=='')
    // {
    //     $this->db->where('S.dealer_id',$this->dealer_id);
    // }
    $this->db->where($cond);
    $this->db->where('S.status','accepted');

    $this->db->group_by('P.model_number');
...
// public function get_admin_incentive_report()
// {
//         $staff=trim($this->input->post('staff',true));
//         $dealer=trim($this->input->post('dealer',true));
//         $outlet=trim($this->input->post('outlet',true));
...
//         { 
//             $cond=$cond." and SD.staff_id=".$staff."";
//         }
//         if($dealer && $dealer !='All Dealer')
//         {  
//             $cond=$cond." and S.dealer_id=".$dealer."";
//         }       
...
//         if($fromdate!='' && $todate!='')
//         {
//             $cond=$cond." and date(S.submit_date) between '". $fromdate."' and '".$todate."'";
//         }
//         elseif($fromdate){
//             $cond=$cond." and date(S.submit_date) between '". $fromdate."' and '".$today."'";
//         }
//         elseif($todate){
//             $cond=$cond." and date(S.submit_date) between '". $today."' and '".$todate."'";
//         }


//         $this->db->select('I.model_id,M.display_name,I.staff_reward_type,T.initial_target_amount,T.incentive,T.final_target_amount,I.start_date,I.end_date,count(S.id) as sales_count');

//         $this->db->from('incentive I');

//         $this->db->join('incentive_target T','I.id = T.incentive_id');

//         $this->db->join('products P','P.model_number = I.model_id');

//         $this->db->join('sales_report S','S.product_id = P.id');

//         $this->db->join('model M','M.id = I.model_id');

//         $this->db->join('dealer_incentive DI','DI.incentive_id = I.id');

//         $this->db->join('staff_dealer SD','DI.dealer_id = SD.dealer_id');

//         $this->db->where("S.submit_date >=I.start_date AND S.submit_date <= I.end_date AND S.status = 'accepted'");
...
//             $this->db->where('S.dealer_id',$this->dealer_id);
//         }
//         $this->db->group_by('I.model_id,I.start_date,T.final_target_amount');   
//         $query = $this->db->get('');
//         if($query->num_rows()>0)
//         {
//             return $query->result();
...
public function get_admin_incentive_report($staff=false)
{
    $model=trim($this->input->post('filtermodel',true));
    $fromdate=trim($this->input->post('filterfromdate',true));
    $todate=trim($this->input->post('filtertodate',true));
    $outletid=trim($this->input->post('filteroutlet',true));
    $userid=trim($this->input->post('filteruserid',true));
    $today=$this->general->get_local_time(); 
     ...
     if($model)
    {
     ...
     if($outletid)
    {
        $cond=$cond." and s.outlet_id=$outletid";
    }
    if($userid)
    {
    ...
    $this->db->select('i.id as incentiveid,i.*,m.display_name,count(s.id) as sales_count');
   
    $this->db->from('sales_report s');
    $this->db->join('products p','s.product_id=p.id');
    $this->db->join('incentive i','p.model_number=i.model_id');
    $this->db->join('model m','m.id = i.model_id','left');
    $this->db->join('dealer_incentive di',"i.id=di.incentive_id ",'left');
    $this->db->where("s.invoice_date BETWEEN i.start_date AND i.end_date and s.status='accepted' and i.end_date>='".$today."'");
    $this->db->where($cond);
...
public function get_admin_incentive_history()
{
    ...
    $this->db->join('incentive i','p.model_number=i.model_id');
    $this->db->join('model m','m.id = i.model_id','left');
    $this->db->join('dealer_incentive di',"i.id=di.incentive_id AND  di.dealer_id = $this->userid",'left');
    $this->db->where("s.invoice_date BETWEEN i.start_date AND i.end_date and s.dealer_id=".$this->userid." and s.status='accepted' and i.end_date<'".$today."'");
    $this->db->where($cond);
    $this->db->group_by('i.id');   
    $this->db->order_by('i.id','desc');
    $query = $this->db->get('');
