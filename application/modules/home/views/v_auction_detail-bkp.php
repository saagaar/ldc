...
<script>
	var aid = '<?php echo $product->host_id; ?>';
	var pid = '<?php echo $product->product_id; ?>';
	var uid = '<?php echo $user_id; ?>'; 
	var reset_time = '<?php echo AUCTION_RESET_TIME; ?>'; //reset time (in seconds). we reset time to this time when timer is less than this time.
	var bid_increment = '<?php echo BID_PRICE_INCREMENT; ?>';
	var bid_time = '<?php echo BID_TIME; ?>';
...
<script>
	...
	socket.on('connect',function(){
		...
		if(uid && uid!='' && uid!='undefined'){
			...
			socket.emit('user_connected',{'auction_room':aid, 'uid':uid,});
		}
		...
		socket.on('display_new_auction_product',function(data){
			...
			//now update auction end date time sring in myVars object
			myVars.auction_end_time = data.auction_end_time;
			
			//alert(sellerInfoTemplate);
			//console.log("Previous Pid :" + pid);
			console.log(aid+' # '+pid+ ' # ' +uid);
			
			//now replace previous pid by new pid in myVars object
			pid = data.product_data.product_id;
...
<script>
	$("#sellerInfo").on("click", "#placeBidBtn", function(e) {
		var bidAmount = $('#bidAmt_' + pid).html();
		...
		console.log('aid:' + aid + ' # pid:' + pid + ' # uid:' + uid + ' # bid_amount:' + bidAmount + ' # bid_increment:' + bid_increment + '# remaining_time:' + remaining_time + ' # reset_time:' + reset_time);
		socket.emit('bid_placed', {'aid':aid, 'pid':pid, 'uid':uid, 'bid_amount':bidAmount, 'bid_increment':bid_increment, 'remaining_time':remaining_time, 'reset_time':reset_time});	
	});
	...
	$('#outgoingChatMessage').keypress(function(event) {
		if(event.which == 13) {
			...
			socket.emit("new_chat_message",{"sender_id":uid,"sender_name":users_full_name,"sender_image":users_image, "aid":aid, "message":$('#outgoingChatMessage').val()});
