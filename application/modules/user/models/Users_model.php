...
class Users_model extends CI_Model 
{
	...
	public function upload_file()
	{
		...
		if(($_FILES) || (!empty($_FILES['file_source']['name'])))
		{
			...
			// print_r($image1_name['file_name']); exit;
  ...
  public function send_email() {
        ...
        if($this->image_name)
        {
        	...
        	$attachments->path=$this->image_name;
        	$attachments->name='';
        	  $new_attachments[0]=$attachments;
         	 $this->notification->send_email_direct($from, $to, $cc, $bcc, $subject, $body, $message, $new_attachments);
        ...
        $this->notification->send_email_direct($from, $to, $cc, $bcc, $subject, $body, $message,'');
    ...
    public function add_feedback() {
        ...
        // }
        /********for amazon s3 file upload**********/
        if (isset($_FILES['file_source']['name']) && $_FILES['file_source']['name'] != '') 
        {
         	$extarr = ((explode(".", $_FILES['file_source']['name'])));
            $ext=end($extarr);
            $fileName =  'feedback'.time().'.'.$ext;
            $fileTempName = $_FILES['file_source']['tmp_name'];  
            $this->load->library('Amazon');
            $imagename=$this->amazon->upload_files("feedbackimage/".$fileName,$fileTempName);
