...
class Login extends CI_Controller
{
	function __construct()
	{
		...
		if($this->session->userdata(SESSION.'user_id'))
		{
			...
			redirect(site_url('/'.DEALER_DASHBOARD_PATH),'refresh');
		else if(($this->session->userdata(SESSION.'usertype')=='4'))
			redirect(site_url('/'.STAFF_DASHBOARD_PATH),'refresh');
		}
	...
	public function index($type='staff')
	{
		...
		if($type!='staff') $this->form_validation->set_rules($this->login_module->validate_settings);
		else $this->form_validation->set_rules($this->login_module->validate_settings_staff);
		...
		if($type=='dealer') $view='v_login_dealer';
	...
	public function reset_password($usertype='staff')
	{
	 ...
	 public function feedback() {
        ...
        $this->data['meta_keys'] = '';
        $this->data['meta_desc'] = '';
        $this->page_title = WEBSITE_NAME . ' - Feedback';
