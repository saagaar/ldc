...
$.validator.addMethod("checkDuplicateEmail", function (value, element) {
	...
	var parentid= $('#parentuserid').val();
	if(parentid)
	{
		var url=urlCheckDuplicateEmail+'/'+parentid;
	}else{
		var url=urlCheckDuplicateEmail;
	}
	$.ajax({
		type : 'POST',
		url : url,
...
$.validator.addMethod("checkDuplicateIMEI", function (value, element) {
	var availability = '';
	...
	if(productid)
	{
		var url=urlCheckDuplicateIMEI+'/'+productid;
	}else{
		var url=urlCheckDuplicateIMEI;
	}
	$.ajax({
		type : 'POST',
		url : url,
...
// $.validator.addMethod("checkDuplicateIMEIedit", function (value, element) {
// 	var availability = '';
// 	var id=$('#productid-prod').val();
// 	$.ajax({
// 		type : 'POST',
// 		url : urlCheckDuplicateIMEIedit+'/'+id,
// 		data : {
// 			imei : value
// 		},
// 		async : false,
...
// }, "IMEI already Exists");
// 
$.validator.addMethod("checkIncentiveDate", function (value, element) {
	var availability = '';
	...
	var dealers=[];
    $('.checkboxindividual:checked').each(function(){
        dealers.push($(this).val());
    })
	

	var parentid= $('#parentincid').val();
	if(parentid)
	{
		var url=checkduplicateIncentivedate+'/'+parentid;
	}else{
		var url=checkduplicateIncentivedate;
	}
	$.ajax({
		type : 'POST',
		url : url,
...
}, "Some of Dealers are already assigned to this incentive period");

$.validator.addMethod("checkDuplicateUsername", function (value, element) {
	var availability = '';
	var parentid= $('#parentuserid').val();
	if(parentid)
	{
		var url=urlCheckDuplicateUsername+'/'+parentid;
	}else{
		var url=urlCheckDuplicateUsername;
	}
	$.ajax({
		type : 'POST',
		url : url,
